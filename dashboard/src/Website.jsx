...
const inventoryCategories = [
  ...
  { category: 'Whole Grains and Rice', amount: '96 items', share: '8%' },
  { category: 'Beverages and Juice', amount: '84 items', share: '7%' },
]
...
function Website() {
  ...
  const [activeRecipeIds, setActiveRecipeIds] = useState(recipes.slice(0, 3).map((recipe) => recipe.id))
  const [availableRecipeIds, setAvailableRecipeIds] = useState(recipes.slice(3).map((recipe) => recipe.id))
  const [expiringList, setExpiringList] = useState(expiringFoods)
  ...
  const activeRecipes = activeRecipeIds
